...
# (length(dataset)-1) original was 7500 and header = FALSE made 7501
# confidence is 0.8 (default)
rules = apriori(dataset, parameter = list(support = (3*7)/(length(dataset)-1), confidence = 0.2, maxlen = 2))

# writing ... [296 rule(s)] done [0.00s].
# 296 rules are passed the conditions which is support = (3*7)/(length(dataset)-1), confidence = 0.2, maxlen = 2

# visualizing the rules top 10 by confidence
